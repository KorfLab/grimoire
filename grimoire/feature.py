...
class mRNA(Transcript):
	...
	def use_longest_orf(self):
		# not sure if this works on negative strand (probably not)
		rna = ''
...
class ncRNA(Transcript):
	...
	`ncRNAs`s are created by first instantiating this class and then using the
	`add_child()` method (from the parent `Feature` class) to add features of
	type 'exon'. **All 'ncRNA`s must have an 'id'.** Children will
	automatically reference this in their 'pid'.
...
class Gene(Feature):
	...
	def is_coding(self):
		"""Returns True if any of the transcripts code for protein."""
		for tx in self.transcripts():
			if tx.is_coding: return True
		return False
