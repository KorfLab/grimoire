...
class TestToolbox(unittest.TestCase):
	...
	def test_GFF_file(self):
		gff = toolbox.GFF_file('data/A.thaliana.1percent.gff3.gz')
		stuff = gff.get(chrom='Chr1', type='exon', beg=5000, end=9999)
		self.assertEqual(len(stuff), 20)

	def test_GFF_stream(self):
		gff = toolbox.GFF_stream('data/C.elegans.1percent.gff3.gz')
		count = 0
		for e in gff:
	...
	def test_FASTA_stream(self):
		ff = toolbox.FASTA_stream('data/A.thaliana.1percent.fasta.gz')
		text = ''
